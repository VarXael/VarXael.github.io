...
function renderDiscipline(role) {
  ...
  discProjects.querySelectorAll('.dp-open').forEach(btn => {
    btn.addEventListener('click', () => openBloom(btn.dataset.id));
  });
...
let mpx = window.innerWidth / 2, mpy = window.innerHeight / 2;   // raw cursor pixels
let cax = mpx, cay = mpy;                                        // smoothed creature position
const trail = [];                                               // bioluminescent wake
const pulses = [];
const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

let targetCX = 0, animCX = 0, animRMult = 1.0;
let animFog = 0.08;   // depth fog eases toward a per-state target
...
/* soft light shafts sinking from above — they thin out as you sink deeper */
function godrays(px, fade) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    grad.addColorStop(0, LUM(0.05 * fade)); grad.addColorStop(1, LUM(0));
    g.fillStyle = grad;
    g.beginPath(); g.moveTo(x - 40, 0); g.lineTo(x + 55, 0); g.lineTo(x - 130, H); g.lineTo(x - 225, H); g.closePath(); g.fill();
  }
}

/* your cursor is a small radiolaria trailing a bioluminescent wake */
function cursorCreature() {
  cax += (mpx - cax) * 0.14; cay += (mpy - cay) * 0.14;
  trail.push({ x: cax, y: cay });
  if (trail.length > 20) trail.shift();
  g.lineWidth = 1.4;
  for (let i = 1; i < trail.length; i++) {
    const a = i / trail.length;
    g.strokeStyle = LUM(a * 0.22);
    g.beginPath(); g.moveTo(trail[i - 1].x, trail[i - 1].y); g.lineTo(trail[i].x, trail[i].y); g.stroke();
  }
  const gl = g.createRadialGradient(cax, cay, 0, cax, cay, 80);
  gl.addColorStop(0, LUM(0.06)); gl.addColorStop(1, LUM(0));
  g.fillStyle = gl; g.beginPath(); g.arc(cax, cay, 80, 0, Math.PI * 2); g.fill();
...
function draw() {
  ...
  let targetRMult = 1.0, targetCamX = 0, targetFog = 0.08;
  if (currentState === 'overview')        { targetCX = W * 0.9;  targetRMult = 2.0; targetCamX = 0;    targetFog = 0.08; }
  else if (currentState === 'discipline') { targetCX = W * 0.86; targetRMult = 1.5; targetCamX = -0.1; targetFog = 0.30; }
  else if (currentState === 'archive')    { targetCX = W * 0.82; targetRMult = 1.4; targetCamX = 0.12; targetFog = 0.52; }
  else                                    { targetCX = W * 0.5;  targetRMult = 1.4; targetCamX = 0.3;  targetFog = 0.72; }

  animCX += (targetCX - animCX) * 0.025;
  animRMult += (targetRMult - animRMult) * 0.025;
  animFog += (targetFog - animFog) * 0.04;
  const heroR = R * animRMult;
  const surfaceFade = Math.max(0, 1 - animFog * 1.2);
  ...
  // your cursor-creature swims on top of everything
  cursorCreature();

  g.globalCompositeOperation = 'source-over';
  schedule();
}
// rAF while the tab is visible; a slow timer when hidden so the scene never freezes
// (and so width-dependent layout stays measured even if the preview renders hidden).
